...
- `frontend/public/tokens.css` defines `--grid-mark` — a tiny SVG (URL-encoded data URI) of a single mark.
- `frontend/public/components.css` `.hud-grid::before` is a fixed, full-viewport overlay (`z-index:-1`) with `background: var(--grid-line)` **masked** by `--grid-mark`, tiled at `mask-size: 44px 44px`.
...
This project picked **rings**. Other site owners may prefer the crosses or the original grid — all are "in the same family" (a fine, structured, low-contrast field).
