...
> **A reference deployment is already live** at `https://avatar-alex.fly.dev` (custom domain `https://avatar.hauptsec.xyz`). The identifiers below — app `avatar-alex`, region `sjc`, domain `avatar.hauptsec.xyz` — are that owner's. **To stand up your own**, pick a globally-unique Fly app name and a region near your Supabase DB, then change them in two places: `APP="..."` in `scripts/deploy.sh` and `app = "..."` / `primary_region = "..."` in `scripts/fly.toml` (keep the two app names in sync). Read every `-a avatar-alex`, `avatar-alex.fly.dev`, and `*.hauptsec.xyz` below as `<your-app>` / `<your-domain>` placeholders.

| | |
|---|---|
| **App** | `avatar-alex` → `https://avatar-alex.fly.dev` |
| **Region** | `sjc` (San Jose) |
...
```bash
fly scale count 2 -a avatar-alex     # min_machines_running=1 keeps 1 warm; soft_limit balances across both
```
...
Run against `https://avatar-alex.fly.dev`. Use `MODEL=openai/gpt-5.4-nano` for cheap test calls if you like, and clean up test data afterwards.
...
- [x] Post a human message from admin → it appears in the visitor's chat within ~10 s (polling), styled as the "live" bubble.
- [x] Contact-capture flow ("I'd like to get in touch", give an email) fires a **Pushover** notification.
...
> `openai/~openai/gpt-mini-latest` (not a valid OpenRouter model ID), which broke every real LLM
> reply in production — only the no-LLM `Qn` shortcut still worked, which is why it's worth
> testing FAQ-tool routing specifically and not just `Qn`. Fixed to `openai/gpt-5.4-mini` via
> `fly secrets set` (no redeploy needed) and reverified. Also fixed a stray `COOKIE_SECURE=0g`
> typo in `.env` — harmless in this case since Fly sources `COOKIE_SECURE` from `fly.toml`'s
> `[env]` block rather than from `.env`, but worth knowing that `.env` typos on non-secret-staged
> keys fail silently rather than erroring.

## 6. Success criteria

Deployment is successful when:
- The app is reachable at `https://avatar-alex.fly.dev` with HTTPS forced, ≥1 machine always running in `sjc`, and the health check green.
...
**Embedding.** A ready-to-paste snippet is in `scripts/wordpress-embed.html` (a WordPress "Custom HTML" block): it pins the app full-bleed just below the site nav, overrides the theme's content-column max-width, guards against horizontal overflow on narrow screens, and forwards `?q=N` from the host page into the iframe. Change two values for your own site — the `BASE` constant (your subdomain, e.g. `https://avatar.<yourdomain>`) and the iframe `title` (your own name). See SPEC-AVATAR.md "Tech stack decisions" for the `frame-ancestors` guidance.

## 8. Operations

- Status / logs: `fly status -a avatar-alex`, `fly logs -a avatar-alex`.
- Scale up/out: `fly scale vm shared-cpu-4x -a avatar-alex` (bigger), `fly scale count 2 -a avatar-alex` (more machines).
- Roll back: `fly releases -a avatar-alex` then `fly deploy` a prior image, or `fly releases rollback -a avatar-alex`.
- Secrets change: `fly secrets set -a avatar-alex KEY=value` (rolling restart).
